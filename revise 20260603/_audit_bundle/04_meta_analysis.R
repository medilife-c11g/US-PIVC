...
if (sum(dwell_idx) >= 2) {
  ...
  forest(m_dwell,
         ...
         xlab = if (i2 > 75) sprintf("NOT POOLED — I² = %.1f%% (exceeds 75%% no-pooling threshold)", i2) else "",
         plotwidth = "8cm",
         col.diamond = "blue")
  dev.off()
...
if (sum(infil_idx) >= 2) {
  ...
  # Sensitivity: REML estimator + Hartung-Knapp-Sidik-Jonkman (HKSJ) adjustment
  # (parallels the catheter-failure sensitivity analysis; small-k robustness check)
  cat("\n--- Infiltration sensitivity: REML + HKSJ ---\n")
  m_infil_hksj <- metabin(
    event.e = df$infiltration_usg[infil_idx], n.e = df$n_usg[infil_idx],
    event.c = df$infiltration_lm[infil_idx], n.c = df$n_lm[infil_idx],
    studlab = labels[infil_idx], sm = "RR", method = "MH",
    method.tau = "REML", hakn = TRUE)
  cat(sprintf("REML+HKSJ infiltration RR = %.2f (95%% CI %.2f-%.2f); p = %.3f\n",
              exp(m_infil_hksj$TE.random), exp(m_infil_hksj$lower.random),
              exp(m_infil_hksj$upper.random), m_infil_hksj$pval.random))
...
if (sum(extrav_idx) >= 2) {
  ...
  forest(m_extrav,
         ...
         print.I2 = TRUE,
         smlab = "Extravasation\nRisk Ratio",
         xlab = if (i2_extrav > 75) sprintf("NOT POOLED — I² = %.1f%% (exceeds 75%% no-pooling threshold)", i2_extrav) else "",
         plotwidth = "8cm")
  dev.off()
...
# illustration.
hr_studies <- c("Feinsmith (2023) — adjusted HR (Cox)",
...
if (length(hr_studies) >= 2) {
  ...
  forest(m_hr,
         ...
         print.I2 = FALSE,
         smlab = "Catheter survival /\npremature removal",
         xlab = "Not pooled — different effect measures (adjusted HR vs RR)",
         xlim = c(0.5, 2.0),
         plotwidth = "8cm")
  dev.off()
