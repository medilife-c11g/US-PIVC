...
The DerSimonian-Laird estimator was the pre-specified primary approach in our PROSPERO protocol, reflecting standard practice in systematic reviews at the time of registration. After the initial analytic run, we recognised that with only 2–4 studies contributing to each outcome, DL standard errors are known to be anti-conservative and the resulting confidence intervals can be narrower than warranted.^15^ We therefore added a *post-hoc* sensitivity analysis using the restricted maximum likelihood (REML) estimator and the Hartung-Knapp-Sidik-Jonkman (HKSJ) confidence interval adjustment, which is the currently recommended approach for small-k random-effects meta-analyses. We retain DL as the pre-specified primary estimator (per PROSPERO) but explicitly flag that, where DL and HKSJ estimates straddle the conventional α = 0.05 threshold (as occurs for catheter failure: DL p = 0.056 vs HKSJ p = 0.047), interpretation should not hinge on either side of that threshold and the overall pattern of evidence should drive clinical inference rather than a single estimator's significance.
...
(See "Primary Outcome: Catheter Failure" above for the *post-hoc* design-stratified analysis [RCT-only vs cohort-only] and the leave-one-out sensitivity analysis showing dependence on the largest cohort study.)
...
Based on current evidence, clinicians should continue to use USG as a valuable tool for achieving venous access in patients with DIVA, where its insertion benefits are well established. However, current evidence does not demonstrate a consistent post-insertion advantage of USG over successful landmark placement, and the available data do not support assuming that the well-documented insertion benefits of USG automatically translate into improved post-insertion catheter performance. To optimise PIVC longevity regardless of insertion technique, attention should be directed toward modifiable factors such as catheter length, catheter-to-vein ratio, securement methods, and post-insertion care bundles. Future RCTs should be adequately powered, control for catheter specifications, employ standardised outcome definitions aligned with the OMG (One Million Global) PIVC outcomes framework, and include follow-up periods sufficient to capture clinically relevant complications.
...
|---------|-------------|--------------|-------------------------|-----------|-------------------|
| Catheter failure | 4 (2 RCTs, 2 cohorts) | 3,404 | RR 1.23 (0.99--1.51) | ⊕⊕◯◯ **Low** | Serious risk of bias (1 high-risk RCT, 2 cohorts); serious imprecision (CI crosses unity) |
...
| Time-to-event-adjacent | 2 (2 cohorts) | 43,800 | Not pooled — Feinsmith adjusted HR 0.91 (0.87--0.95); Shokoohi 72-h RR 1.26 (0.88--1.80); different effect measures, narrative synthesis only | ⊕◯◯◯ **Very low** | Different effect measures (HR vs cumulative-incidence RR); serious risk of bias (observational); conflicting directions; serious imprecision |
...
**Figure 2.** Forest plot: catheter failure or premature removal, USG vs landmark PIVCs (k = 4: 2 RCTs + 2 cohort studies). Random-effects (DerSimonian-Laird) pooled estimate shown; the borderline signal (RR 1.23, 95% CI 0.99–1.51, p = 0.056) is driven primarily by the cohort component (see Results, Catheter Failure). PIVC type and intravascular catheter-segment length are reported in Table 1 where available; standard short PIVCs predominate in the four pooled studies.

**Figure 3.** Forest plot: catheter dwell time (USG vs landmark), continuous outcome (k = 3: 2 RCTs + 1 cohort). The pooled diamond is intentionally suppressed under the pre-specified I² > 75% rule (I² = 91.9%); only individual study estimates are shown. Cottrell 2021 used identical catheter types in both groups but disproportionately classified USG cases as difficult; the other two unconfounded studies (Kleidon 2025; Leroux 2023) used standard short PIVCs and showed no dwell-time difference. Studies in which the USG arm received systematically longer catheters than the landmark arm (Refosco 2024; Desai 2018; Dachepally 2023; Paladini 2018) are reported in "Studies with major confounding" rather than in this forest plot.

**Figure 4.** Forest plot: infiltration (USG vs landmark), k = 2 (1 RCT + 1 cohort, both standard-PIVC). Random-effects pooled RR 0.68 (0.12–3.83); CI is very wide because both studies have low event counts and small samples.

**Figure 5.** Forest plot: catheter failure subgroup by age (adult vs paediatric, k = 4 same as Figure 2). The adult subgroup contains 2 cohorts + 1 conference abstract (Saltarelli 2015 dominates the adult weight); the paediatric subgroup contains a single RCT (Kleidon 2025). Subgroup-by-effect interaction not statistically significant (p = 0.32) and severely underpowered.

**Figure 6.** Individual study estimates (NOT pooled) for catheter survival / premature removal: Feinsmith 2021 (adjusted HR from multivariable Cox model, 30-day follow-up) and Shokoohi 2019 (cumulative-incidence RR at fixed 72-hour follow-up). The pooled diamond is intentionally suppressed because HR and cumulative-incidence RR are conceptually different effect measures and pooling them on the log-HR scale would mix incompatible quantities.
